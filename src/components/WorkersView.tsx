...
export default function WorkersView({ workers, setWorkers }: { workers: Worker[]; setWorkers: React.Dispatch<React.SetStateAction<Worker[]>> }) {
  ...
  const handleAdd = () => {
    ...
    setWorkers(prev => [{ id: Date.now(), name: name.trim(), phone: phone.trim(), job: job.trim(), dailyWage: parseFloat(dailyWage) || 0 }, ...prev]);
    ...
    <div className="p-8 max-w-5xl mx-auto" dir="rtl">
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-slate-700/50 overflow-hidden">
        {workers.length === 0 ? <div className="p-16 text-center text-slate-400">لا يوجد عمال مسجلين بعد.</div> : (
          ...
          <tbody className="divide-y divide-slate-700/50">{workers.map((w, i) => (<tr key={w.id} className="hover:bg-slate-800/30 transition-colors"><td className="px-6 py-4 text-slate-500">{i + 1}</td><td className="px-6 py-4 text-white font-medium">{w.name}</td><td className="px-6 py-4 text-slate-300 font-mono" dir="ltr">{w.phone}</td><td className="px-6 py-4 text-slate-300">{w.job}</td><td className="px-6 py-4 text-emerald-400 font-bold font-mono">{w.dailyWage.toFixed(3)} د.ك</td><td className="px-6 py-4"><button onClick={() => setWorkers(prev => prev.filter(x => x.id !== w.id))} className="text-red-400 hover:text-red-300 text-lg">×</button></td></tr>))}</tbody></table>
        )}
      </div>
    </div>
  );
